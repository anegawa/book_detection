...
STRIDE = 0
STRIDE_NUM = 1
STRIDE_TRES = int(STRIDE_NUM/2) + 1

DIFF_METHOD = "cnt" #"cnt" or "all"
METHOD_NAME = ['Correlation', 'Chi-square', 'Intersection', 'Bhattacharyya distance']
METHOD_INDEX = 3
METHOD = int(environ.get('METHOD', METHOD_INDEX))

WIDE_RANGE = 40
#SHIFTの方法：中心を動かさず角度を動かす＝angle 角度は買えずに中心を左右にずらす＝loc
SHIFT = "loc" #"loc" or "angle"
SHIFT_FLAG = False
HOSEI_FLAG  = True
HOSEI_DIFF_X = 10
HOSEI_DIFF_Y = 10

#deg_flagを使うかどうか（例：False　使わない)
DEG = False
...
# THRESHOLD = 4000
if METHOD_INDEX == 3:
    THRESHOLD = 0.4 #0.4 0.25(20190210に変更)
elif METHOD_INDEX == 2:
    THRESHOLD = 1000
else:
    sys.exit(1)
DRAW_FLAG = True
DRAW_RED_ONLY = True
DRAW_HOSEI_ONLY = True
b = ""
if DRAW_FLAG or DRAW_RED_ONLY:
    b = "_test"
elif DRAW_HOSEI_ONLY:
    b = "_adj"

SHIFT_NORM = 10
DIFFERENT_CNT_FLAG = False
DIFFERENT_CNT_NUM = 20
DIFFERENT_CNT_DIFF = 1

DIFFERENT_ANGLE_FLAG = False
DIFFERENT_ANGLE_NUM = 17
DIFFERENT_ANGLE = 10

del_mammer = "hen" #area or hen


TODAY = str(datetime.datetime.today().strftime("%Y%m%d"))
print("day & time : " + str(datetime.datetime.today().strftime("%Y/%m/%d/%H/%M/%S")))
DETAIL = DIFF_METHOD + "_tres_each_" + str(THRESHOLD) + "_" + "str" + str(STRIDE) + "_" + str(STRIDE_NUM)
# TODAY = '20190206'
if SHIFT_FLAG:
    a = "shift_" + SHIFT
elif DIFFERENT_CNT_FLAG:
    a = "CNT"
elif DIFFERENT_ANGLE_FLAG:
    a = "ANGLE"
elif HOSEI_FLAG:
    a = "all_shift" + SHIFT + "_" + str(HOSEI_DIFF_X) + str(HOSEI_DIFF_Y)
else:
    a = ""
VERSION = "test"
VERSION = 'Bhattacharyya_' + TODAY + "_" + DETAIL + PLAIN_STR + "_" + range_method + "_" + str(WIDE_RANGE) + "_" + a + "_" + del_mammer + b

# VERSION = 'intersection_' + TODAY
NET_NAME = 'resnet_v1_101'
# NET_NAME = 'voc_80001model'
...
# INFERENCE_IMAGE_PATH = ROOT_PATH + '/tools/cut4ks'
...
# elif NET_NAME.startswith('voc_80001model'):
#     weights_name = 'voc_80001model'
else:
    raise NotImplementedError

#これはresnetのckpt
# PRETRAINED_CKPT = ROOT_PATH + '/data/pretrained_weights/' + weights_name + '.ckpt'
PRETRAINED_CKPT = os.path.join(ROOT_PATH, 'output/trained_weights/part4/voc_80001model.ckpt')
# PRETRAINED_CKPT = ROOT_PATH + '/output/trained_weights/part4/' + weights_name + '.ckpt'
# TRAINED_CKPT = os.path.join(ROOT_PATH, 'output/trained_weights/')
TRAINED_CKPT = os.path.join(ROOT_PATH, 'output/trained_weights/part4/')
...
IMG_MAX_LENGTH = 2000
# IMG_SHORT_SIDE_LEN = 3400
# IMG_MAX_LENGTH = 7000
CLASS_NUM = 1

# --------------------------------------------- Network_config
BATCH_SIZE = 1 #今は１だけなんだって

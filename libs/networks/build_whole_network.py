...
class DetectionNetwork(object):
    ...
    def postprocess_fastrcnn_r(self, rois, bbox_ppred, scores, img_shape):
        ...
        with tf.name_scope('postprocess_fastrcnn_r'):
            ...
            for i in range(1, cfgs.CLASS_NUM+1):
                ...
                # 3. NMS
                # print("~~~~~~~")
                # print(tmp_decoded_boxes)
                # print(tmp_score)
                # print(tmp_decoded_bo.shape)
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_r_batch, gtboxes_h_batch):
        ...
        print("l:402 img_shape : "  + str(img_shape))
    ...
    def get_restorer(self):
        # path_join = os.path.join(cfgs.TRAINED_CKPT, cfgs.VERSION)
        # print(path_join)
        # checkpoint_path = tf.train.latest_checkpoint(path_join)

        checkpoint_path = tf.train.latest_checkpoint(cfgs.TRAINED_CKPT)
        print("checkpoint path : " + str(checkpoint_path))
        if checkpoint_path != None:
            ...
            # for key, item in list(restore_variables.items()):

    ...
    for i, dir_name in enumerate(dirs):
        ...
        for j in range(len(files)):
            ...
            if os.path.exists(a_file):
                ...
                # print(img[:,:,2].shape)
                # print(img[:][0].shape)
                ax.plot_wireframe(X, Y, img[:,:,0])
                plt.savefig("./3Dgraphs/" + dir_name + "_" + str(j) + "_0.jpg")
                fig = plt.figure()
                ax = Axes3D(fig)
                ax.plot_wireframe(X, Y, img[:,:,1])
                plt.savefig("./3Dgraphs/" + dir_name + "_" + str(j) + "_1.jpg")
                fig = plt.figure()
                ax = Axes3D(fig)
                ax.plot_wireframe(X, Y, img[:,:,2])
                plt.savefig("./3Dgraphs/" + dir_name + "_" + str(j) + "_2.jpg")
                # cv2.imwrite("./3Dgraphs/" + dir_name + "_" + str(j) + ".jpg",img)
                fig = plt.figure()
                ax = Axes3D(fig)
                ax.plot_wireframe(X, Y, img[:,:,0]+img[:,:,1]+img[:,:,2])
                plt.savefig("./3Dgraphs/" + dir_name + "_" + str(j) + "_sum.jpg")
                ...
                #                             iro = img[int(y)][i]
                #                             print("color : " + str(iro))
                #                             if iro[0] < 150 and iro[1] < 150 and iro[2] < 150:
                #                                 black_count += 1
